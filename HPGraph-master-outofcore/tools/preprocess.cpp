...
void generate_edge_grid(std::string input, std::string output, VertexId vertices, int partitions, int edge_type) {
	...
	printf("%ld\n",vertices*sizeof(unsigned long));
	//for(unsigned int i=0;i<vertices;i++)
		//vertex_l[i]=0;
	memset(vertex_l,0,vertices*sizeof(unsigned long));
...
*/
	...
	FILE* f_w=fopen((output+"/record_c").c_str(),"w+");

	printf("%ld\n",vertices*sizeof(unsigned long));
	fwrite(vertex_l,vertices,sizeof(unsigned long),f_w);
